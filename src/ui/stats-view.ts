/**
 * Statistics View UI for API Gateway Plugin
 */
...
async function showByProvider(
  ...
  const choice = await ctx.ui.select("按供应商查看", [...providers, "返回"]);
  ...
  const lines = [
    ...
    `缓存命中率：${cacheRate}%`,
    `花费：$${providerStats.cost.toFixed(2)}`,
  ];
